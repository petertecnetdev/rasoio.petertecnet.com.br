...
export default function OrderSelectedClientCard({ client, onClear }) {
  ...
  const safeItem = useMemo(
    () => ({
      ...safeClient,
      name: `${safeClient.first_name || ""} ${safeClient.last_name || ""}`.trim(),
      avatar: safeClient.avatar,
      images: safeClient.images || null,
    }),
    [safeClient]
  );
    ...
    <Card className="mt-3">
      <Card.Body className="d-flex align-items-center gap-3">
        ...
        <div className="flex-grow-1">
          ...
          {safeClient.email && (
            <div className="text-muted small">{safeClient.email}</div>
          )}
          {safeClient.phone && (
            <div className="text-muted small">{safeClient.phone}</div>
          )}
          {safeClient.cpf && (
            <div className="text-muted small">{safeClient.cpf}</div>
          )}
        </div>
