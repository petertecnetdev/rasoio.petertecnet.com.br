...
import { FaBoxOpen, FaBriefcase, FaCut, FaMapMarkerAlt, FaStore } from "react-icons/fa";
...
export default function GlobalCard({ item, fmtBRL, navigate, showSchedule, openSchedulePopup, actions }) {
  ...
  const isProduct = safeItem.type === "product" || safeItem.is_product === true || safeItem.isProduct === true || !!safeItem.product_id || !!safeItem.productId || safeItem.item_type === "product";
  ...
  const cardKind = isEstablishment ? "establishment" : isEmployer ? "employer" : isProduct ? "product" : "service";
  const kindMeta = {
    establishment: { label: "Barbearia", icon: <FaStore /> },
    employer: { label: "Barbeiro", icon: <FaCut /> },
  ...
  const establishmentLogo = useMemo(() => {
    const paths = [establishment?.images?.logo, establishment?.logo, establishment?.image, establishment?.images?.background];
    for (const p of paths) {
      const url = imageUrl(p);
      if (url) return url;
    }
  ...
  const detailsPath = useMemo(() => {
    ...
    if (isEmployer) {
      const userName = safeItem.user_name || safeItem.username || safeItem.user?.user_name || safeItem.user?.username || null;
  ...
  const handleDetails = () => detailsPath && navigate(detailsPath);
  ...
  return (
    <article ref={cardRef} className={`carousel-card hologram-container type-${safeItem.type || "item"} card-kind-${cardKind}`}>
      <div className="globalcard-kind">{kindMeta.icon}<span>{kindMeta.label}</span></div>

      <div className={`carousel-image-wrap ${isEmployer ? "img-round" : isEstablishment ? "img-establishment" : "img-square"}`} onClick={handleDetails} role={detailsPath ? "button" : undefined}>
        <img src={image && !mainImageBroken ? image : placeholderSvg} alt={safeItem.name || kindMeta.label} loading="lazy" className="carousel-image" onError={(e) => { baseHandleImgError(e); setMainImageBroken(true); }} />
      </div>

      <div className="carousel-item-content">
        <div className="carousel-item-name" onClick={handleDetails} role={detailsPath ? "button" : undefined} title={safeItem.name || ""}>{safeItem.name}</div>

        {!isEstablishment && establishment?.name && (
          <div className="globalcard-establishment" role="button" onClick={handleEstablishmentClick} title={establishment.name}>
            {establishmentLogo && !establishmentLogoBroken && <img src={establishmentLogo} alt={establishment.name} className="globalcard-establishment-logo" onError={(e) => { baseHandleImgError(e); setEstablishmentLogoBroken(true); }} />}
            <span className="globalcard-establishment-name">{establishment.name}</span>
          </div>
        )}

        {(safeItem.city || safeItem.uf) && <div className="globalcard-location"><FaMapMarkerAlt size={12} className="globalcard-location__icon"/><span className="globalcard-location__text">{safeItem.city}{safeItem.uf ? ` - ${safeItem.uf}` : ""}</span></div>}
        ...
        <div className="carousel-item-metrics">
          {safeItem.metrics && <>
            <Badge bg="secondary" className="px-2 py-1 rounded-pill">{safeItem.metrics.total_views ?? 0} visualizações</Badge>
            {"completed_orders" in safeItem.metrics && <Badge bg="secondary" className="px-2 py-1 rounded-pill">{safeItem.metrics.completed_orders ?? 0} concluídos</Badge>}
          </>}
        </div>

        <div className="carousel-item-actions">
          {canSchedule && <GlobalButton size="sm" full variant="primary" stopPropagation onClick={() => openSchedulePopup(safeItem)} className="carousel-btn">Agendar</GlobalButton>}
...
GlobalCard.propTypes = { item: PropTypes.object, fmtBRL: PropTypes.func, navigate: PropTypes.func, showSchedule: PropTypes.bool, openSchedulePopup: PropTypes.func, actions: PropTypes.node };
GlobalCard.defaultProps = { fmtBRL: (v) => v, showSchedule: false, openSchedulePopup: null };
