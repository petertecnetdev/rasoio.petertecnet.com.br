...
export default function OrderEditPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setProducts(resItems.data.map((p) => ({ ...p, price: Number(p.price) })));
  ...
  const handleAddItem = async () => {
    ...
    const getHtml = (currentCat) => `
      <div class="order-modal">
        ...
        <div class="order-modal__items-grid row p-2">
    ...
    await Swal.fire({
      ...
      didOpen: () => {
        ...
        const htmlContainer = popup.querySelector('.order-modal__content-fullscreen');

        const attachAddListeners = () => {
          htmlContainer.querySelectorAll('.order-modal__item-add').forEach((btn) => {
            btn.addEventListener('click', (e) => {
              const id = Number(e.currentTarget.getAttribute('data-id'));
              const prod = products.find((p) => p.id === id);
              if (!prod) return;
              const parsedProd = { ...prod, price: Number(prod.price) };
              setOrderLines((lines) => [
                ...lines,
                { product: parsedProd, quantity: 1, additions: [], removals: [] },
              ]);
              Swal.close();
            });
          });
        ...
        htmlContainer.querySelectorAll('.order-modal__tab').forEach((tab) => {
          tab.addEventListener('click', () => {
            selectedCategory = tab.getAttribute('data-cat');
            Swal.update({ html: getHtml(selectedCategory) });
            setTimeout(attachAddListeners, 0);
          });
        });
  ...
  const handleManage = async (index, type) => {
    ...
    let selected = type === 'additions' ? orderLine.additions : orderLine.removals;
    ...
    await Swal.fire({
      ...
      html: `<div style="padding:0.8rem 0.6rem"><div class="order-modal__item-list" style="max-height:350px;overflow-y:auto;">${itemsHtml}</div></div>`,
      showCancelButton: true,
      ...
      preConfirm: () => {
        if (type === 'additions') {
          const arr = [];
          document.querySelectorAll('.order-modal__addition-qty').forEach((el) => {
            const qty = parseInt(el.value, 10);
            if (qty > 0) {
              arr.push({ id: Number(el.getAttribute('data-id')), quantity: qty });
            }
          });
          return arr;
        }
        return Array.from(document.querySelectorAll('.order-modal__removal-check:checked')).map(
          (el) => Number(el.value)
        );
      },
    }).then((res) => {
