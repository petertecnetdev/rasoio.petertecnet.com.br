...
function toIsoDate(value) {
  ...
  if (!d) return "";
  // Corrige para timezone de São Paulo (mantém coerência de data local)
...
export default function EmployerDashboardPage() {
  ...
  const recomputeSummary = useCallback((list) => {
    ...
    const todayCount = list.filter(
      (a) => toIsoDate(a.order_datetime) === todayISO
    ).length;
    const tomorrowCount = list.filter(
      (a) => toIsoDate(a.order_datetime) === tomorrowISO
    ).length;
  ...
  const computeNexts = useCallback((list) => {
    ...
    const onlyPendingUpcoming = upcoming.filter(
  ...
  const loadInitial = useCallback(async () => {
    try {
      ...
      const { data: me } = await safeAxios((req) =>
        req.get(`${apiBaseUrl}/auth/me`)
      );
      meRef.current = me || {};
      ...
      const baseList = Array.isArray(listResp.appointments)
        ? listResp.appointments
        : [];
      ...
      try {
        ...
        fullOrders = arr.filter(
          (o) => (o.type || "appointment") === "appointment"
        );
      } catch {
        fullOrders = baseList;
      }
  ...
  const pushNotification = useCallback(
    (title, body, kind = "info", payload = {}) => {
      const id = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      setNotifQueue((q) =>
        [{ id, title, body, kind, payload }, ...q].slice(0, 6)
      );
      setTimeout(() => {
  ...
  const handleFinalize = async (appt, status) => {
    try {
      setLocalLoading(true);
  ...
  const checkUpdates = useCallback(async () => {
    try {
      ...
      try {
        ...
        const base = Array.isArray(listResp.appointments)
          ? listResp.appointments
          : [];
 ...
 const timeButtons = useMemo(() => {
  ...
  const filteredAppointments = useMemo(() => {
    const source =
      Array.isArray(allOrders) && allOrders.length ? allOrders : appointments;
    ...
    <div className="dashboard-root">
      ...
      <Container fluid className="dashboard-container">
        ...
        {finalizableAppointment && (
          <Card className="glass-card mb-4 border-warning">
            <Card.Body>
              ...
              <div className="text-light">
                O atendimento de <b>{finalizableAppointment.customer_name}</b>{" "}
                (pedido{" "}
                <Badge bg="dark">#{finalizableAppointment.order_number}</Badge>)
                foi concluído e precisa ser finalizado.
              </div>
              <div className="d-flex gap-2 mt-3">
                <Button
                  ...
                  variant="success"
                  onClick={() =>
                    handleFinalize(finalizableAppointment, "attended")
                  }
                >
                  <CheckCircle className="me-1" /> Marcar como Atendido
                </Button>
                <Button
                  size="sm"
                  variant="danger"
                  onClick={() =>
                    handleFinalize(finalizableAppointment, "not_attended")
                  }
                >
                  <XCircle className="me-1" /> Marcar como Não Atendido
                </Button>
              </div>
            </Card.Body>
          </Card>
            ...
            <ButtonGroup className="filters-status flex-wrap">
              ...
              {STATUS_KEYS.filter((s) => s !== "all").map((s) => (
                <Button
                  ...
                  size="sm"
                  variant={
                    filters.status === s ? "light" : "outline-light"
                  }
                  className={`btn-chip ${
            ...
            <ButtonGroup className="filters-time flex-wrap">
              ...
              {timeButtons.map((b) => (
                <Button
                  ...
                  size="sm"
                  variant={
                    filters.day === b.key ? "light" : "outline-light"
                  }
                  className={`btn-chip ${
                  ...
                  <>
                    ...
                    <div className="d-flex gap-2 mt-3">
                      <Button
                        ...
                        variant="success"
                        onClick={() =>
                          handleAction(nextPendingAppointment, "confirm")
                        }
                      >
                      ...
                      <Button
                        ...
                        variant="danger"
                        onClick={() =>
                          handleAction(nextPendingAppointment, "cancel")
                        }
                      >
                        ...
                        <Badge
                          bg={
                            nextAppointment.appointment_status === "pending"
                              ? "warning"
                              : nextAppointment.appointment_status ===
                                "confirmed"
                              ? "info"
                              : "secondary"
                          }
                        ...
                        <Badge
                          className={
                            lastAppointment.appointment_status === "pending"
                              ...
                              : lastAppointment.appointment_status ===
                                "confirmed"
                  ...
                  recentAppointments.map((a) => (
                    ...
                    >
                      ...
                      <div className="recent-item__body">
                        {weekdayPt(a.order_datetime)} —{" "}
                        {shortPt(a.order_datetime)} às{" "}
                  ...
                  <Card className="appointment-card">
                    <Card.Body>
                      ...
                      {Array.isArray(a.services) && a.services.length > 0 && (
                        <div className="appointment-card__row">
                          <span className="appointment-card__label">
                            Serviços
                          </span>
                          <span className="appointment-card__value">
