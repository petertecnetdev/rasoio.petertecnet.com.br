    ...
    const fetchBarbershop = async () => {
      try {
        ...
        Swal.fire({
          icon: "error",
          title: "Erro!",
          text: error.response?.data?.message || "Erro ao carregar informações da barbearia.",
        });
      } finally {
        setLoading(false);
      }
    ...
    <>
      ...
      {loading ? (
        <ProcessingIndicatorComponent 
          messages={["Carregando dados da barbearia...", "Verificando barbeiros...", "Quase pronto!"]} 
        />
      ) : (   
              ...
              <Card.Body className="barbershop-body">
                <Row className="barbershop-row">
                  <Col md={6} className="barbershop-info">
                    ...
                    <p className="barbershop-address">{barbershop.address}, {barbershop.city} - {barbershop.state}</p>
                    <div className="barbershop-actions">
                      <Button variant="primary" onClick={() => navigate(`/scheduling/create/${barbershop.slug}`)}>Agendar</Button>
                      <Button variant="primary" onClick={() => window.open(barbershop.location, "_blank")}>Localização</Button>
                      <Button variant="primary" onClick={() => window.open(barbershop.instagram, "_blank")}>Instagram</Button>
                      <Button variant="primary" onClick={() => window.open(`https://wa.me/${barbershop.phone}?text=Olá,%20gostaria%20de%20saber%20mais%20sobre%20os%20serviços%20da%20${barbershop.name}.`, "_blank")}>
