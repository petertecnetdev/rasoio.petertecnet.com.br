...
// Cores para linhas de tabela (desktop)
const rowClasses = {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const res = await axios.get(`${apiBaseUrl}/appointment/listmy`, {
          headers: { Authorization: `Bearer ${token}` },
        });

        // flatten e extrai slugs
...
await axios.patch(
  ...
  const filtered = appointments.filter((a) => {
    ...
    if (filterDate && a.scheduled_at.slice(0, 10) !== filterDate)
      return false;
    return true;
  });
                  ...
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((a) => (
                      <tr key={a.id} className={rowClasses[a.status] || ""}>
                        ...
                        <td>
                          {a.provider_slug ? (
                            <Link to={`/barber/view/${a.provider_slug}`}>
                              {a.provider_name}
                            </Link>
                          ) : (
                            a.provider_name
                      ...
                      <Card bg={variant.bg} text={variant.text}>
                        <Card.Body>
                          ...
                          <Card.Text>
                            ...
                            {a.provider_slug ? (
                              <Link
                                to={`/barber/view/${a.provider_slug}`}
                                className={
                                  variant.text === "white" ? "text-white" : ""
                                }
                              >
                                {a.provider_name}
                              </Link>
